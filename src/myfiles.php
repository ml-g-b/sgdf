      ...
      <div id="content_container">
        <div id="formulaire">
          <form method="POST" action="ajout.php" enctype="multipart/form-data">
            <fieldset>
              <legend>Ajout</legend>
              <input type="file" id="input" name="input" accept="application/pdf" required><br>
              <label><input type="radio" id="public" value="1" name="visibility">Public</label><br>
              <label><input type="radio" id="private" value="0" name="visibility" checked>Privé</label><br>
        ...
        <div>
        <?php
            $stmt=$pdo->prepare("SELECT * FROM files WHERE idF IN(SELECT idF FROM possess WHERE idU IN (SELECT id FROM user WHERE name=?))");
